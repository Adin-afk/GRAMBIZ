    ...
    <div id="page-main-content" className="flex min-h-screen items-center justify-center bg-bg px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-xs text-ink-muted">{t("common.tagline")}</p>
        </div>

        <form onSubmit={handleSubmit} className="border border-border bg-surface p-6">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-ink-muted">{t("auth.signIn")}</h2>
          ...
          <label className="mb-5 block text-sm">
            <span className="mb-1 block text-ink-muted">{t("auth.password")}</span>
            <input
              type="password"
              required
              value={password}
